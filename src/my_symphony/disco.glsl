...
void main(void)
{
  ...
  float freq   = 0.75*texture2D(iChannel0,vec2(uv.x,0.25)).x;
  float wave   = texture2D(iChannel0,vec2(uv.x,0.75)).x;
  float freqc  = smoothstep(0.0,(1.0/iResolution.y), freq + uv.y - 0.9);
  float wavec  = smoothbump(0.0,(1.0/iResolution.y), wave + uv.y - 0.25);
  gl_FragColor = vec4(0.50, freqc /1.5,  wavec / 3, 0.5);
}
