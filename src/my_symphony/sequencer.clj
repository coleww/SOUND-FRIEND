...
(stop)
(@bazes :probs)
;(def last-prob-avg (atom 0.33))
;(t/start-fullscreen "src/my_symphony/disco.glsl" :textures ["textures/0.png" "textures/1.png" "textures/2.png" :overtone-audio] :user-data {"iProbs" last-prob-avg})
(println @hevvys)
;(t/stop)
...
(do
  (def key-siggy (atom {:tonic :F3 :scale :minor :bpm 152}))
  ...
  (defn maybe-play
    ...
    (let [instroo @instru
          notations @notes
          prob (get-current notations :probs)
      ...
      (if (and (pos? (count prob)) (< (rand) (get prob idx)))
        (if (get-current notations :notes)
          (->
           ...
           (get idx)
           (rand-nth)
           (me/int-2-freq (:tonic @key-siggy) (:scale @key-siggy))
           (hz->midi)
           (instroo idx))
          (instroo idx)))))

  (def metro (metronome (:bpm @key-siggy)))
